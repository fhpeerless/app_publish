...
function handleCheckResult(result) {
    ...
    if (!cardData.is_active || cardData.activate_time === null || cardData.expire_time === null) {
        ...
        // 卡密已激活，使用后端返回的剩余时间
        console.log(`剩余天数: ${cardData.remaining_days}, 剩余小时数: ${cardData.remaining_hours}`);
        
        // 根据剩余时间设置显示
        if (cardData.remaining_days > 0) {
            // 超过1天，显示剩余天数和小时数
            if (cardData.remaining_hours > 0) {
                resultRemainingDays.textContent = `${cardData.remaining_days} 天 ${cardData.remaining_hours} 小时`;
            } else {
                resultRemainingDays.textContent = `${cardData.remaining_days} 天`;
            }
        } else if (cardData.remaining_hours > 0) {
            // 不足1天，显示剩余小时数
            resultRemainingDays.textContent = `${cardData.remaining_hours} 小时`;
        } else {
            // 已过期
            resultRemainingDays.textContent = '0 小时';
        }
        
        if (cardData.remaining_days > 0 || cardData.remaining_hours > 0) {
